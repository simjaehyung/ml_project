...
## 9. Pool B의 이론적 근거와 구조적 한계

### Pool B가 존재하는 이유 — 이론 지형

Pool B("신규 Flexi 구매자")는 데이터에서 관측되지 않는 반사실적(counterfactual) 수요다.
이 수요의 존재와 행동 방향은 아래 네 가지 이론으로 정당화된다.

| 이론 | 저자 / 연도 | Pool B에 대한 함의 |
|------|------------|------------------|
| **Probabilistic Selling** | Fay & Xie, *Marketing Science* 2008 | 가격 민감 + 속성 선호가 약한 소비자는 불확실한 상품을 선택한다. Flexi Rate는 이 이론이 예측하는 "probabilistic good"의 호텔 버전이다. |
| **Screening / Self-Selection** | Rothschild & Stiglitz (1976); Mussa & Rosen (1978) | Standard vs. Flexi 두 계약 제시 시 소비자 타입별 분리 균형(separating equilibrium)이 성립한다. Flexi 선택자 = 일정 유연 타입이 자기선택 → `flexi_noshow_rate`가 낮게 유지되는 이론적 근거. |
| **Prospect Theory** | Kahneman & Tversky (1979) | 이득 프레임("€18 절약") 유지 시 전환율이 손실 프레임("방 미확정")보다 유의미하게 높다. 우리의 커뮤니케이션 설계가 이 조건을 충족한다. |
| **Mechanism Design** | Myerson (1981, Nobel 2007) | Flexi Rate 시스템은 유인 양립적 메커니즘이다. 호텔이 타입을 묻지 않아도 소비자가 자신의 진짜 타입을 드러내는 것이 지배 전략이 된다. Pool B는 조작된 수요가 아니라 메커니즘이 자연 발생시키는 수요다. |

### 구조적 한계 — 이론으로도 막을 수 없는 것

이 이론들은 Pool B의 **존재와 방향**을 정당화하지만, **크기(magnitude)**는 정당화하지 못한다.

```
이론이 말하는 것
  "price-sensitive, flexible 소비자는 Flexi를 산다"  ← 방향 ✅

이론이 말하지 않는 것
  "이 호텔, 이 시즌, 이 할인율에서 몇 %가 전환하는가"  ← 수치 ❌
```

`conversion_rate`는 **반사실적 수요**다 — 우리 데이터셋은 실제로 예약한 사람의 기록이다. Flexi 상품을 보고 사지 않은 사람의 기록은 존재하지 않는다. 이것은 보수적인 값을 쓰더라도 해결되지 않는다. "왜 0.60인가"도 "왜 0.85인가"와 같은 질문에 노출된다.

**이 한계는 프로젝트 범위 내에서 해결 불가능한 구조적 약점이다.**

### 발표 방어 전략 — 약점을 인정하되 프레임을 지킨다

발표에서 이 약점을 먼저 꺼내는 것이 방어력을 높인다.

```
"저희 시뮬레이션은 두 레이어로 구성됩니다.

 레이어 1 (Pool A) — 완전히 데이터 기반
   실제 테스트셋의 취소 결과를 사용합니다.
   PR-AUC → walk_rate 관계는 이 레이어에서 나옵니다.
   이것이 핵심 주장입니다.

 레이어 2 (Pool B) — 이론 기반 + 민감도 분석
   신규 Flexi 구매자의 전환율은 Fay & Xie (2008)의
   probabilistic selling 이론이 존재를 뒷받침하지만,
   정확한 수치는 데이터가 없습니다.
   0.60~0.90 범위의 sensitivity analysis를 수행했으며,
   이 전 범위에서 시스템은 net positive입니다.
   현장 검증이 필요한 파라미터임을 인정합니다."
```

이 포지셔닝의 핵심: **"우리가 모르는 것을 안다"는 것을 보여주는 것이 신뢰도다.**
